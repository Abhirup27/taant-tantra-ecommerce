...
export const Shiprocket_serviciability_schema = z.object({
  ...
  data: z.object({
    available_courier_companies: z.array(z.object({
      ...
      others: jsonStringToObject.pipe(
        z.object({
...
export async function createCronService(valkeyClient: GlideClient) {
  ...
  async function get_servicibility() {
    try {
      ...
      const response = await fetch(config.SHIPROCKET_BASE_URL + 'external/courier/serviceability?pickup_postcode=201306&delivery_postcode=122002&cod=0&weight=1', {
        method: "GET",
        headers: headers,
      })
      if (!response.ok) {
